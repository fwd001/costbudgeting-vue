...
export default {
  data () {
    return {
      ...
      rules: {
        ...
        costValue: [{ required: true, message: '请输入成本值', trigger: 'blur' }]
      },
      rules2: {
        projectId: [{ required: true, message: '请选择项目', trigger: 'change' }],
  ...
  methods: {
    ...
    ruleForm1Modify () {
      ...
      const o1 = this.ruleForm1
      let id = this.listId
      modifyPublishCost({id, ...o1}).then(res => {
        this.dialogLoading1 = false
        if (res.code !== -404) {
          this.dialogVisible = false
          this.$message({
    ...
    tableDelete1 (id, index) {
      ...
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        deletePublishCost(data).then(res => {
          if (res.code !== -404) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    getPBList (data) {
      ...
      getPublishCostList(data).then(res => {
    ...
    getPBList2 (data) {
      ...
      getPublishCostList2(data).then(res => {
    ...
    cellStyle2 ({row, column, rowIndex, columnIndex}) {
      ...
      if (column.property !== 'contentName' && column.property !== 'projectName' && column.property !== 'date') {
